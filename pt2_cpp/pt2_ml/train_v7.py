...
parser.add_argument("--skip_shap",   action="store_true",
                    help="Skip SHAP (slow on large test sets)")
...
class Pt2Dataset(IterableDataset):
    ...
    def __iter__(self):
        for f in self.files:
            ...
            is_real = ak.to_numpy(data["is_real"])

            if is_real.sum() == 0:
                continue

            idx = np.arange(len(is_real))
            np.random.shuffle(idx)

            def arr(name):
                return ak.to_numpy(data[name])[idx]

            ls_phi  = arr("ls_phi")
            pls_phi = arr("pls_phi")

            md0_dxy = arr("pt2_md0_dxy")
            log_abs_md0_dxy = np.log(np.abs(md0_dxy) + LOG_EPS)

            features = np.stack([
                arr("ls_pt"),  arr("ls_eta"),
                np.sin(ls_phi), np.cos(ls_phi),
                arr("pls_pt"), arr("pls_eta"),
                np.sin(pls_phi), np.cos(pls_phi),
                arr("pls_charge"), arr("pls_nhit"),
                arr("pt2_delta_pt"),  arr("pt2_delta_eta"),
                arr("pt2_delta_phi"), arr("pt2_delta_R"),
                md0_dxy,              arr("pt2_md0_dz"),
                arr("pt2_md1_dxy"),   arr("pt2_md1_dz"),
                arr("pt2_md0_rz"),    arr("pt2_md1_rz"),
                log_abs_md0_dxy,                         # NEW
            ], axis=1)
            ...
            for i in range(0, len(y), BATCH_SIZE):
                yield (
                    torch.tensor(features[i:i+BATCH_SIZE], dtype=torch.float32),
                    torch.tensor(y[i:i+BATCH_SIZE],        dtype=torch.float32)
                )
...
# Load files
# ---------------------------
files = sorted(glob.glob(os.path.join(args.data_dir, "*.root")))
random.shuffle(files)

if args.fast_dev_run:
    train_files = files[:10]
    val_files   = files[10:11]
    test_files  = files[11:12]
else:
    train_files = files[:140]
    val_files   = files[140:160]
    test_files  = files[160:]
...
norm_loader = DataLoader(Pt2Dataset(train_files), batch_size=None)
all_feats = []
...
train_loader = NormWrapper(DataLoader(Pt2Dataset(train_files), batch_size=None))
val_loader   = NormWrapper(DataLoader(Pt2Dataset(val_files),   batch_size=None))
test_loader  = NormWrapper(DataLoader(Pt2Dataset(test_files),  batch_size=None))
...
for x, y in DataLoader(Pt2Dataset(test_files), batch_size=None):
